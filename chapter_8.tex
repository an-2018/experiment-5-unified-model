...
Large language models have been applied to depression detection via instruction-tuned text encoders and multi-modal audio-language models for paralinguistic features \citep{zhao2025multimodalllm}. We evaluate LLM-enhanced encoders in our ablation track (levels L1--L5, where L0 = classical encoders) but focus this chapter on the classical encoder results.
...
The DAIC-WOZ corpus \citep{gratch2014distress} contains 189 clinical interviews with PHQ-8 depression labels. Prior work spans graph-based text models on transcribed clinical interviews \citep{burdisso2024gcndp}, multiple-instance learning \citep{zhang2025mil}, and hierarchical graph-attention fusion of text and audio \citep{niu2021multimodal}. Reported metrics vary widely by evaluation protocol (F1 vs.\ AUROC, inclusion of therapist/interviewer prompts, differing label thresholds), so we do not attempt a leaderboard-style numeric comparison and instead report AUROC under our own fixed, leakage-safe protocol throughout.
...
MOSEI is 120$\times$ larger than DAIC. Without mitigation, MOSEI would dominate every training batch. We address this with temperature-balanced sampling (T=3.0) as described in Section~\ref{sec:training_setup}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{paper/figures/model_architecture.png}
    \caption{Unified Multimodal Graph-Gated MoE Architecture. Datasets (DAIC-WOZ, CMU-MOSEI, ChaLearn FI) feed into modality-specific encoders (text: RoBERTa, audio: WavLM, video: ViT), routed per dataset (text-only for DAIC, video-only for FI, multimodal gated fusion for MOSEI). The MMoEEx expert bank (8 experts, hidden dimension 256) is routed by both a task-specific gate and a GraphSAGE/GAT router, combined in log-space with routing weight $\lambda=0.5$. Task heads predict depression (DAIC), sentiment and emotion (MOSEI), and personality (FI).}
...
Training uses AdamW optimizer with initial learning rate $3\times10^{-4}$, cosine annealing schedule, and early stopping (patience=20 epochs on validation loss, max 150 epochs). Modality projectors are frozen for the first 20 epochs, then the top 2 projector layers are unfrozen for the remainder of training (this is the mechanism that plays the role a general learning-rate warmup would elsewhere). All experiments use a single NVIDIA GPU.

The multitask objective is an unweighted sum of the four task losses (binary cross-entropy for depression and emotion; NLL regression loss for sentiment and personality). The two regression losses additionally use a homoscedastic-uncertainty NLL form \citep{kendall2017uncertainties} with a single learned $\sigma$ shared across both, rather than a separate $\sigma_t$ per task:
...
\textbf{Temperature-balanced sampling}: To prevent MOSEI (16,265 samples) from dominating DAIC (107 samples) in joint training, we sample with temperature $T=3.0$:
...
Figure~\ref{fig:training_pipeline} details the full implementation of this training procedure: the DataLoader's per-batch tensor shapes, model initialization, the epoch loop (forward pass, loss, optimizer step), and the negative-transfer monitor and progressive-unfreezing mechanisms described above.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{paper/figures/pipeline-flow.png}
    \caption{Implementation detail of Phases 5--11: the non-graph MMoEEx baseline (Phase 5, the comparison point used throughout Section~\ref{sec:graph_results}) and the graph-routed joint multitask training (Phase 7) share the same DataLoader, initialization, and epoch training loop, differing only in whether the graph router contributes to expert routing; downstream phases (LLM ablations, domain adaptation, calibration, explainability) follow.}
...
\begin{itemize}
    \item \textbf{DAIC}: Video (AUROC=0.5823) is the strongest modality, followed by text (AUROC=0.5346) and audio (AUROC=0.4686). However, all three AUROC values are close to random chance (0.5) and have widely overlapping 95\% CIs, reflecting the challenging small-sample nature of this dataset ($n=107$ training). Audio underperformance is consistent with prior findings that acoustic features alone are often insufficient for depression detection.
\end{itemize}
...
Table~\ref{tab:mmoeex_results} shows MMoEEx results compared to the best standalone baselines. MMoEEx uses 8 experts (2 shared, 6 exclusive), NLL loss for regression, and temperature-balanced sampling (T=3.0).
...
We evaluated unsupervised domain adaptation to transfer knowledge from MOSEI and FI to DAIC depression detection. Four methods were tested against a no-adaptation baseline: CORAL \citep{sun2017deepcoral}, MMD, DANN \citep{ganin2015dann,ganin2016dann}, and a combined CORAL+MMD loss. All methods were evaluated on three transfer scenarios: FI$\to$DAIC, MOSEI$\to$DAIC, and multisource (both FI and MOSEI).
...
Across all LLM ablation pairs (L1--L5 vs L0), no comparison reached statistical significance at $\alpha=0.05$, reflecting the limited statistical power from the small DAIC validation set ($n=35$). The largest effect sizes are observed for L4 vs L0 ($d=0.170$) and L1 vs L0 ($d=0.152$). We recommend larger-sample replication to establish clinical significance.
