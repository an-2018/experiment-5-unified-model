...
\begin{enumerate}
    \item \textbf{Graph-gated MoE routing}: We propose combining KNN graph construction with GraphSAGE-based expert selection, demonstrating that topology-aware expert routing consistently improves over non-graph baselines across three affective computing benchmarks and four tasks.
...
\begin{enumerate}
    \item \textbf{Graph routing improves multimodal multitask learning}. The KNN graph + GraphSAGE router consistently improves over non-graph MMoEEx, with V0 achieving MOSEI CCC=0.6803 (+0.18) and V3 achieving DAIC AUROC=0.8967 (+0.40). This demonstrates that topology-aware expert selection can benefit cross-dataset affective computing under a leakage-safe evaluation protocol.
