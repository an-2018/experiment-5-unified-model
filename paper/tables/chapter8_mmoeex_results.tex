\begin{table}[htbp]
\centering
\caption{MMoEEx Results vs Best Standalone Baselines. MMoEEx uses 8 experts (2 shared, 6 exclusive), NLL loss, temperature-balanced sampling (T=2.0). Per-dataset routing: DAIC→text-only, MOSEI→multimodal, FI→video-only. See \S\ref{sec:mmoeex_results}. Source: \texttt{artifacts/tables/mmoe\_ex\_results.csv}.}
