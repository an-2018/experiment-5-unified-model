...
\begin{abstract}
    We investigate whether graph-guided mixture-of-experts routing improves unified multimodal mental health assessment, jointly learning depression detection (DAIC-WOZ), sentiment/emotion recognition (CMU-MOSEI), and personality prediction (ChaLearn First Impressions) under a single model with modality-specific encoders, gated late fusion, an MMoEEx expert bank, and a leakage-safe KNN-graph GraphSAGE router. Our non-graph baseline is a real, working depression classifier (DAIC AUROC 0.573, consistent with a 0.671 unimodal-text-only ablation) after fixing a sampling bug that had silently capped an earlier version of it at chance level (Section~\ref{sec:results}). Against this working baseline, across five leakage-safe graph configurations, graph routing \emph{consistently underperforms} on both DAIC (AUROC 0.471--0.551 vs.\ 0.573) and FI personality (CCC 0.478--0.531 vs.\ 0.571), and shows a small, consistent \emph{edge} on MOSEI (sentiment CCC 0.486--0.512 vs.\ 0.493; emotion AUROC 0.713--0.742 vs.\ 0.703). A graph-homophily diagnostic explains this per-task split: the routing graph carries no depression-label signal at all, so no router could help there regardless of baseline strength; it carries real signal for sentiment/emotion and personality, which the router now measurably converts into a gain for MOSEI but still fails to exploit for FI, where it instead degrades performance. Neither a learned per-task routing weight nor a $K$-sensitivity sweep recovers a DAIC benefit. We report this as a rigorously investigated, task-dependent negative result. Separately, cross-attention fusion does not reproduce a previously reported gain over gated fusion, and LLM-based encoders show a similarly task-dependent pattern: substantial point-estimate gains on MOSEI sentiment/emotion, a DAIC gain only when every modality is LLM-based, and a uniform loss on FI personality — none of the DAIC comparisons reach statistical significance at this sample size. We release a leakage-safe graph construction protocol and a full ablation ladder.
\end{abstract}
...
Our contributions are fourfold. First, a rigorously investigated, task-dependent negative result: graph-guided gating that combines GraphSAGE-aggregated neighbourhood information with task-specific gates in log-space consistently fails to beat a non-graph baseline on depression detection and personality prediction, while giving a small, consistent gain on sentiment/emotion recognition; we support this with a homophily diagnostic and two targeted fix attempts rather than a bare negative number. Second, a leakage-safe graph construction protocol with three modes (inductive, split-local, transductive) under strict subject-independent splits, with an explicit audit that no test-label leakage occurs through graph edges. Third, a systematic ablation ladder — unimodal baselines, three fusion strategies, an MMoEEx baseline, five graph-routing configurations, and LLM-encoder replacements — showing LLM-based encoders give the largest point-estimate gains we observed, though not yet at statistically significant levels at this sample size. Fourth, a diagnosed and fixed sampling bug that had silently capped our own non-graph baseline at chance-level DAIC performance (Section~\ref{sec:results}) — a concrete illustration of why baseline correctness must be verified before drawing routing conclusions.
...
\textbf{Multimodal fusion} for affective computing spans early fusion (feature concatenation), late fusion (learned per-modality gates), and low-rank or cross-attention fusion. Gated late fusion with learned modality gates and Low-Rank Multimodal Fusion \citep{zadeh2017LMF} avoid the parameter cost of full tensor fusion. Cross-modal attention has also been proposed for depression detection; we do not observe a benefit from it under our evaluation protocol (Section~\ref{sec:results}).
