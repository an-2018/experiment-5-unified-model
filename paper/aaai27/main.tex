...
\caption{Unified multimodal graph-gated MoE architecture.}
...
We instantiate $M{=}8$ expert MLPs with residual connections. For the graph-routed configuration (our primary reported results), all 8 experts are available to every task via a full per-task softmax gate $g_t(h_i) \in \mathbb{R}^8$ (expert hidden dimension 128); we additionally report a non-graph MMoEEx baseline that hard-isolates experts per task group (hidden dimension 256), used to quantify the value added by graph routing.
...
Depression (DAIC) uses a binary head with BCE loss; sentiment (MOSEI) a regression head with NLL loss; emotion (MOSEI, 6 labels) a multi-label BCE head; personality (FI, Big-Five) five regression heads with NLL loss. The multitask objective combines per-task losses with learned homoscedastic uncertainty weights \citep{kendall2017uncertainties}:
\[
\mathcal{L} = \sum_t \tfrac{1}{2\sigma_t^2}\mathcal{L}_t + \log\sigma_t + \lambda_{\text{excl}}\mathcal{L}_{\text{excl}}.
\]
...
AdamW ($\text{lr}{=}1\times10^{-3}$), cosine annealing, early stopping (patience 10 epochs, max 150). Batch size 32. Temperature-balanced sampling ($T{=}2.0$) prevents MOSEI (120$\times$ larger than DAIC) from dominating joint training. Projectors are frozen for the first 20 epochs, then partially unfrozen.
