...
\caption{5-seed mean $\pm$ std: learned per-task $\lambda$ (V0 configuration) reaches DAIC AUROC $0.546\pm0.034$ against the fixed-$\lambda{=}0.5$ V0 baseline's $0.518\pm0.027$ — a delta of $+0.028$ that is \textbf{not reportable} under SPEC-H4-03's stricter effect-size criterion, despite a naive paired $t$-test alone giving $p{=}0.007$. FI CCC moves from $0.480\pm0.060$ (fixed $\lambda$) to $0.501\pm0.047$ (learned), also not reportable. A single-seed comparison (one training run each) previously read learned $\lambda$ as producing ``a real gain on DAIC'' (AUROC $0.489 \to 0.525$); Table~\ref{tab:reversal_catalogue} documents this as one of six such reversals.}
