...
\begin{itemize}
    \item \textbf{LLM encoders universally improve over classical encoders} (L0): Every LLM level (L1--L5) improves every task metric over L0. The gains range from +0.04 (FI, L5) to +0.13 (DAIC, L1).
    \item \textbf{L1 (Mistral-LoRA text)} provides the largest DAIC gain: AUROC=0.6775 (+0.1304 over L0), demonstrating that LLM text features capture richer depression-relevant cues than RoBERTa.
    \item \textbf{L3 (LLM video)} achieves the best MOSEI sentiment (CCC=0.6292) and FI (Avg CCC=0.5704), showing that LLM-based video encoders are particularly beneficial for sentiment and personality.
    \item \textbf{L2 (LLM audio)} achieves the best MOSEI emotion (AUROC=0.7645), suggesting audio LLMs capture paralinguistic emotional cues well.
...
\begin{figure}[htbp]
    ...
    \caption{SHAP beeswarm plot for DAIC: modality-level contributions across 20 synthetic samples. Text shows the highest variance and positive mean contribution, consistent with the model's text weight (3.0) being highest. See \texttt{artifacts/figures/phase\_11\_xai/} for per-dataset versions.}
...
\textbf{LLM encoders improve all tasks}: Replacing classical encoders with LLM-based encoders (Mistral-LoRA for text, audio-LLM, video-LLM) consistently improves performance across all metrics. L1 (Mistral-LoRA text) yields the largest DAIC gain (+0.1304 AUROC). However, no LLM level surpasses the V0 graph router, confirming that graph routing and LLM encoders provide complementary benefits.
...
\begin{itemize}
    ...
    \item Calibration and XAI evaluation are pending Phase 10 and Phase 11 completion.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{LLM encoders universally improve over classical encoders}, but no LLM level (L1--L5) surpasses graph routing. Mistral-LoRA text alone (L1, DAIC AUROC=0.6775) provides the largest gain, while adding visual and audio LLMs (L5, DAIC AUROC=0.6667) slightly dilutes the text benefit.
