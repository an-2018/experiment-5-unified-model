...
\begin{itemize}
    ...
    \item \textbf{L1 (Mistral-LoRA text)} provides the largest DAIC gain: AUROC=0.6775 (+0.1847 over L0), demonstrating that LLM text features capture richer depression-relevant cues than RoBERTa.
...
Despite these contributions, several limitations remain. The DAIC training set ($n=107$) is a fundamental bottleneck; even graph routing cannot fully compensate for limited clinical data. MOSEI audio and video features remain incompletely preprocessed in our pipeline. The LLM ablation levels L6--L9 (multimodal LLM with domain-specific pretraining) could not be evaluated due to computational constraints. Graph routing increases inference complexity relative to non-graph alternatives, which may limit real-time deployment. Finally, FI apparent personality is not equivalent to clinical depression; the auxiliary supervision framing, while valid, has inherent construct validity limitations.
